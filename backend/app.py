...
# ── Logging setup ─────────────────────────────────────────────────────────────
# LOG_LEVEL     — app logs (request boxes, timing)          default: INFO
# SDK_LOG_LEVEL — SDK logs (handshake, decrypted payloads)  default: INFO
#
...
t1_sdk = VahanaCryptoSdk(SERVER_PRIV,   InMemoryCryptoSessionStore(), log_level=_SDK_LOG_LEVEL)
t2_sdk = VahanaCryptoSdkV2(SERVER_PRIV, InMemoryCryptoSessionStore(), log_level=_SDK_LOG_LEVEL)
...
def _proto(sdk) -> str:
    return 'T1' if isinstance(sdk, VahanaCryptoSdk) else 'T2'

def _log_req(sdk, session_id: str) -> float:
    proto = _proto(sdk)
    ep    = _endpoint(request.path, proto)
    logger.info(
        f'┌─ {_tag(proto)} {_BOLD}{request.method} {ep}{_RST}'
...
def _log_res(t0: float, result: dict) -> None:
    ...
    (logger.info if ok else logger.error)(f'└─ {status}  {_DIM}{elapsed:.0f}ms{_RST}\n')
...
def _handle(sdk, handler_fn):
    ...
    enc    = _encrypt_response(sdk, sid, result)
    _log_res(t0, result)
    return enc
...
def _make_stream(sdk):
    ...
    def generate():
        for i in range(repeat_count):
            ...
            logger.info(f'│  {_MGN}~{_RST}  chunk {i + 1}/{repeat_count}  {_DIM}{chunk["content"][:60]}{_RST}')
            yield f"data: {json.dumps(enc)}\n\n"
            time.sleep(0.4)
        elapsed = (time.monotonic() - t0) * 1000
        logger.info(f'└─ {_GRN}stream done{_RST}  {_DIM}{repeat_count} chunks · {elapsed:.0f}ms{_RST}\n')
        yield "data: [DONE]\n\n"
...
def t1_handshake():
    return jsonify(t1_sdk.do_handshake(request.get_json()))
...
def t2_handshake():
    return jsonify(t2_sdk.do_handshake(request.get_json()))
